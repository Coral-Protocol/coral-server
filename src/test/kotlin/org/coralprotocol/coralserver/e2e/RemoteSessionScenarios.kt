...
import org.coralprotocol.coralserver.utils.*
...
//            )
    ...
    fun testRemoteAgentProxying(): Unit = runBlocking {
        ...
        shouldNotThrowAny {
            mockkConstructor(PaymentConfig::class)
            every { anyConstructed<PaymentConfig>().walletAddress } returns "test wallet address"
            mockkStatic(::createBlockchainService)
            coEvery { createBlockchainService(any()) } returns mockk<BlockchainServiceImpl>() {
                coEvery {
                    createEscrowSession(any(), any(), anyNullable())
                } returns Result.success(mockk<SessionInfo>() {
                    every { sessionId } answers { 3000L }
                })

                coEvery {
                    getEscrowSession(any())
                } returns Result.success(mockk<Session>() {
        ...
        val exportingSideExportedGraphAgent = GraphAgent(
            registryAgent = RegistryAgent(
                ...
                unresolvedExportSettings = mapOf(
                    RuntimeId.FUNCTION to UnresolvedAgentExportSettings(
                        quantity = 1u,
                        pricing = RegistryAgentExportPricing(0L, 10000)
                    )
                )
            ),
            description = "",
        ...
        val importingServerSession =
            importingServer.getSessionManager()
                .getOrCreateSession("test", "aaa", "aaa", null, incomingSessionInfo = ps!!)
        ...
        //TODO: Test making payment claim
        // TODO Test session is ended
    }
}
