 ...
 */
fun Routing.publicRoutes(appConfig: AppConfig, sessionManager: SessionManager) {
    get("/api/v1/registry") {
        val registry = appConfig.registry?.map { entry -> entry.value.toPublic(entry.key.toString()) } ?: listOf();
