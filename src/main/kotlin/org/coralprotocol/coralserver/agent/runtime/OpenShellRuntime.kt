...
data class OpenShellRuntime(
    ...
    override suspend fun execute(
        ...
        val egressPolicy = compileEgressPolicy(
            declared = executionContext.registryAgent.execution,
            coralUrls = buildSet {
                add(applicationRuntimeContext.getApiUrl(AddressConsumer.CONTAINER))
                add(applicationRuntimeContext.getMcpUrl(transport, executionContext, AddressConsumer.CONTAINER))
                executionContext.graphAgent.proxies.keys.forEach { name ->
                    add(applicationRuntimeContext.getLlmProxyUrl(executionContext, AddressConsumer.CONTAINER, name))
                }
            },
        )
        val policyDir = writePolicyFiles(egressPolicy, config, coralIp)
    ...
    private suspend fun warnOnSupervisorVersionMismatch(
        ...
        if (!versionChecked.add("$supervisor|$expected")) return
...
private val versionChecked = ConcurrentHashMap.newKeySet<String>()

// Pin a /32 only when the operator configured a literal, non-loopback IP.  A hostname (including the
// host.docker.internal Docker Desktop alias) resolves differently inside the container's netns, so the caller
// falls back to the broad bridge ranges rather than risk a /32 the agent can't reach.
internal fun resolveCoralIp(address: String): String? {
    val ip = runCatching { InetAddress.getByName(address) }.getOrNull() ?: return null
    return address.takeIf { ip.hostAddress == it && !ip.isLoopbackAddress }
}
